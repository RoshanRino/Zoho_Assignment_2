...
public:
    ...
    void possibleMovement(int i,int j)
    {
        ...
        {
            if(board[i+2][j+1]->getType()=="white") cout<<place(i+2,j+1)<<" Take Piece "<<endl;
        ...
        {
            if(board[i+1][j+2]->getType()=="white") cout<<place(i+1,j+2)<<" Take Piece "<<endl;
        ...
        if(i-2<8&&i-2>-1&&j+1<8&&j+1>-1 && board[i-2][j+1]->getType()!=board[i][j]->getType())
        {
            if(board[i-2][j+1]->getType()=="white") cout<<place(i-1,j+1)<<" Take Piece "<<endl;
            else cout<<place(i-2,j+1)<<endl;
            possible.push_back(place(i-2,j+1));
        }
    }
